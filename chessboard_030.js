...
// запустите анонимную область
;(function() {
'use strict';

// Функции шахматной утилиты

var COLUMNS = 'abcdefgh'.split('');

function validMove(move) {
  // перемещение должно быть строкой
  if (typeof move !== 'string') return false;

  // ход должен быть в форме "e2-e4", "f6-d5"
  var tmp = move.split('-');
...
function validFen(fen) {
  ...
  // отсеките любую информацию о ходах, рокировках и т.д. с конца
  // нас интересует только информация о местоположении
  fen = fen.replace(/ .+$/, '');

  // Поле должно состоять из 8 секций, разделенных косыми чертами
  var chunks = fen.split('/');
  if (chunks.length !== 8) return false;

  // проверьте секции деталей
  for (var i = 0; i < 8; i++) {
    if (chunks[i] === '' ||
        chunks[i].length > 8 ||
...
//------------------------------------------------------------------------------
// Переменные области действия модуля
//------------------------------------------------------------------------------

// Элементы Объектных моделей документа
var containerEl,
  boardEl,
  draggedPieceEl,
  sparePiecesTopEl,
...
// возвращает значение true, если версия >= минимальная
...
function error(code, msg, obj) {
  // ничего не делайте, если параметр showErrors не установлен
...
function checkDeps() {
  //если containerId представляет собой строку, это должен быть идентификатор узла DOM (Элементы Объектных моделей)
  if (typeof containerElOrId === 'string') {
    ...
    // убедитесь, что элемент контейнера существует в DOM
  ...
  // в противном случае это должно быть что-то,
  // что становится коллекцией jQuery с размером, т.е. один узел DOM или объект jQuery
  else {
  ...
  // проверьте наличие совместимой версии jQuery
...
// проверка конфигурации / установка параметров по умолчанию
function expandConfig() {
  ...
  // ориентация по умолчанию - белая
  if (cfg.orientation !== 'green ') {
    cfg.orientation = 'pink';
  }
